...
import type { KeyStats } from '@/api/stats'
...
export default function KeyUsageBarChart({ data }: { data: KeyStats[] }) {
  if (data.length === 0) {
  ...
  const chartData = data.map((k) => ({
    ...k,
    errorRate: k.request_count > 0 ? (k.error_count / k.request_count) * 100 : 0,
  }))
      ...
      <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 30 }}>
        ...
        <Tooltip
          ...
          formatter={(_v, _n, props) => {
            ...
            return [`${d.request_count} req, ${d.error_count} err, ${d.avg_duration_ms}ms avg`, d.api_key_name]
          }}
        />
        <Bar dataKey="request_count" radius={[0, 4, 4, 0]} name="Requests">
          {chartData.map((entry) => (
            <Cell key={entry.api_key_id} fill={getBarColor(entry.errorRate)} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
